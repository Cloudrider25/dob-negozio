...
export default async function GoalDetailPage({ params }: { params: PageParams }) {
  ...
  const media =
    goal.cardMedia && typeof goal.cardMedia === 'object' && 'url' in goal.cardMedia
      ? goal.cardMedia
      : null
  const imageUrl = media && typeof media.url === 'string' ? media.url : null
  const imageAlt =
    media && typeof media.alt === 'string' ? media.alt : goal.boxName || t.services.title
  ...
  const cardDescriptionText = (() => {
    if (!goal.cardDescription || typeof goal.cardDescription !== 'object') return ''
    const root = (goal.cardDescription as { root?: unknown }).root
    if (!root) return ''
    const parts: string[] = []
