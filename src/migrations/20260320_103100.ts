...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    ALTER TABLE "pages_locales"
      ADD COLUMN IF NOT EXISTS "shipping_content" jsonb;

    INSERT INTO "pages" ("page_key", "hero_title_mode", "hero_style", "updated_at", "created_at")
    SELECT
      'shipping'::"public"."enum_pages_page_key",
    ...
    WHERE NOT EXISTS (
      SELECT 1
      FROM "pages"
      WHERE "page_key" = 'shipping'::"public"."enum_pages_page_key"
    );
    ...
    SELECT
      ...
      p."id"
    FROM "pages" p
    WHERE p."page_key" = 'shipping'::"public"."enum_pages_page_key"
      AND NOT EXISTS (
        SELECT 1
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
    ...
    ALTER TABLE "pages_locales"
      DROP COLUMN IF EXISTS "shipping_content";
  `)
}
