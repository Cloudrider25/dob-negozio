...
export function NavigatorGrid({
  ...
  const [clickedArea, setClickedArea] = useState<
    Area | undefined
  >(undefined);
  const [isSlideOutAnimating, setIsSlideOutAnimating] =
    useState(false);
  const [hoveredTreatment, setHoveredTreatment] =
    useState<Treatment | null>(null);
  const [clickedTreatment, setClickedTreatment] = useState<
    Treatment | undefined
  >(undefined);
  const [
    isTreatmentSlideOutAnimating,
    setIsTreatmentSlideOutAnimating,
  ] = useState(false);
  const [hoveredGoal, setHoveredGoal] = useState<Goal | null>(
    null,
  );
  const [clickedGoal, setClickedGoal] = useState<
    Goal | undefined
  >(undefined);
  const [isGoalSlideOutAnimating, setIsGoalSlideOutAnimating] =
    useState(false);
        ...
        <div className="col-span-9 relative">
          {state.step === "area" ? (
            ...
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="col-span-2 h-[440px] mt-[34px]">
                <CenterImageDisplay
                  hoveredArea={hoveredArea}
                  shouldSlideOut={isSlideOutAnimating}
                  onAnimationComplete={
                    handleImageSlideOutComplete
                  }
                />
              </div>
            </div>
                  ...
                  {state.step === "goal" &&
                    state.selectedArea && (
                      <GoalHoverCard
                        ...
                        onAnimationComplete={
                          handleGoalCardSlideOutComplete
                        }
                      />
                    )}
                  {state.step === "treatment" && (
                    <TreatmentHoverCard
                      ...
                      onAnimationComplete={
                        handleTreatmentCardSlideOutComplete
                      }
                    />
                  )}
                </div>
