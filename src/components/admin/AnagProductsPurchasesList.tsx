...
export default function AnagProductsPurchasesList() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/order-items?${params.toString()}`, { credentials: 'include' })
        if (!res.ok) throw new Error(`Order items request failed: ${res.status}`)
        const data = (await res.json()) as ApiListResponse
        if (cancelled) return

        const nextRows: Row[] = (data.docs ?? []).map((doc) => {
          const order = (doc.order && typeof doc.order === 'object' ? (doc.order as ApiDoc) : null) ?? null
          const product = (doc.product && typeof doc.product === 'object' ? (doc.product as ApiDoc) : null) ?? null
          return {
            id: asId(doc.id),
            orderId: asId(doc.order),
            ...
            productTitle: asString(product?.title) || 'Prodotto',
          }
        })
        ...
        if (!cancelled) {
          setError(e instanceof Error ? e.message : 'Errore caricamento prodotti acquistati')
          setRows([])
        }
      } finally {
  ...
  if (!anagraficaId) return <div style={{ color: 'var(--theme-text)' }}>Salva l&apos;anagrafica per vedere gli acquisti.</div>
  if (loading) return <div style={{ color: 'var(--theme-text)' }}>Caricamento prodotti acquistati…</div>
  if (error) return <div style={{ color: 'var(--theme-error-500, #d14343)' }}>{error}</div>

  return (
    <div style={{ display: 'grid', gap: '0.75rem' }}>
      {rows.length === 0 ? (
        <div style={{ color: 'var(--theme-text)' }}>Nessun prodotto acquistato per questo cliente.</div>
      ) : (
        <div
          style={{
            display: 'grid',
          ...
          {rows.map((row) => (
            ...
            >
              <div style={{ minWidth: 0 }}>
                <div style={{ fontWeight: 600, color: 'var(--theme-text)' }}>{row.productTitle}</div>
      ...
      <div style={{ color: 'var(--theme-elevation-700)', fontSize: '0.9rem' }}>
        Totale storico prodotti: <strong>{formatMoney(total)}</strong>
      </div>
    </div>
  )
}
