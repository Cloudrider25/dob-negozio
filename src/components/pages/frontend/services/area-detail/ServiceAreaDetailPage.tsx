...
export default async function AreaDetailPage({ params }: { params: PageParams }) {
  ...
  const media = resolveMedia(area.cardMedia, area.name || t.services.title)
  const imageUrl = media?.url ?? null
  const imageAlt = media?.alt || area.name || t.services.title
  const cardDescriptionText = getPlainTextFromRichText(area.cardDescription)
