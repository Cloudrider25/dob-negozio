...
import { getPlainTextFromRichText } from '@/components/shared/richtext'
...
export default async function GoalDetailPage({ params }: { params: PageParams }) {
  ...
  const media = resolveMedia(goal.cardMedia, goal.boxName || t.services.title)
  const imageUrl = media?.url ?? null
  const imageAlt = media?.alt || goal.boxName || t.services.title
  const cardDescriptionText = getPlainTextFromRichText(goal.cardDescription)
