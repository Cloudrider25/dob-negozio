...
import { getShopBaseData, getShopPageConfig, getShopRoutineData } from '@/lib/frontend/shop/getShopData'
import { buildContactLinks } from '@/lib/frontend/contact/links'
import { Hero } from '@/frontend/components/heroes/Hero'
import { ShopSectionSwitcher } from '@/frontend/page-domains/shop/pages/shop-page/sections/ShopSectionSwitcher'
import type {
  NeedData,
  ProductCard,
  TextureData,
} from '@/frontend/page-domains/shop/pages/shop-page/sections/shop-navigator.types'
import type { ShopRouteParams, ShopSearchParams } from '@/frontend/page-domains/shop/pages/shop-page/internal/contracts'
import {
  buildMediaUrl,
  getRelationId,
  ...
  toIdArray,
} from '@/frontend/page-domains/shop/pages/shop-page/internal/shared'
import type {
  RoutineStep,
  RoutineStepRule,
  RoutineTemplate,
...
export default async function ShopPage({
  ...
  const { query, brand, sort, section, perPage, page, view } =
    await parseShopSearchParams(searchParams)
  ...
  const routineNeeds = needsResult.docs.map((need) => ({
    id: String(need.id),
    label: need.name || '',
    description: resolveLocalizedText(need.description, locale),
  ...
  const productAreas = productAreasResult.docs.map((area) => ({
    ...
    description: resolveLocalizedText(area.description, locale),
  }))
  ...
  const routineTimings = timingProductsResult.docs.map((timing) => ({
    ...
    description: resolveLocalizedText(timing.description, locale),
  }))
  ...
  const routineSkinTypes = skinTypesResult.docs.map((skinType) => ({
    ...
    description: resolveLocalizedText(skinType.description, locale),
    productAreaId:
  ...
  const brandsById = new Map(
    brandsResult.docs.map((brandDoc) => [
      String(brandDoc.id),
      {
        ...
        label: resolveLocalizedText(brandDoc.name, locale) || '',
  ...
  const brandLinesById = new Map(
    brandLinesResult.docs.map((brandLineDoc) => [
      String(brandLineDoc.id),
      {
        ...
        label: resolveLocalizedText(brandLineDoc.name, locale) || '',
  ...
  const routineSteps = routineStepsResult.docs.map((step) => {
    ...
    return {
      id: String(step.id),
      label: resolveLocalizedText(step.name, locale) || '',
  ...
  const routineTemplates = routineTemplatesResult.docs.map((template) => {
    ...
    return {
      ...
      name: resolveLocalizedText(template.name, locale) || '',
      description: resolveLocalizedText(template.description, locale) || undefined,
