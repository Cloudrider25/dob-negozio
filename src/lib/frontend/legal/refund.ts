...
const fallbackByLocale: Record<Locale, string> = {
  it: '<p>Politica resi e rimborsi in aggiornamento.</p>',
  en: '<p>Refund policy is being updated.</p>',
  ru: '<p>Политика возврата обновляется.</p>',
}
...
export const getRefundConfig = cache(async (locale: Locale): Promise<RefundConfig> => {
  try {
    ...
    const html = resolveRichTextHtml(page?.refundContent)
    ...
    console.error(`[refund] Failed to load page config for locale "${locale}".`, error)
