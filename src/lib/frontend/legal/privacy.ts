...
const fallbackByLocale: Record<Locale, string> = {
  it: '<p>Informativa privacy in aggiornamento.</p>',
  en: '<p>Privacy policy is being updated.</p>',
  ru: '<p>Политика конфиденциальности обновляется.</p>',
}
...
export const getPrivacyConfig = cache(async (locale: Locale): Promise<PrivacyConfig> => {
  try {
    ...
    const html = resolveRichTextHtml(page?.privacyContent)
    ...
    console.error(`[privacy] Failed to load page config for locale "${locale}".`, error)
